...
GLfloat rectangle_x[NUM_RECTANGLE][4] = { {56.96,58.48,60.26,58.96},{58.48,58.46,60.42,60.26},{58.54,58,60,60.42} ,{58,57.22 ,58.92 , 60},{57.22,55.96,57.02,58.92},{55.96,54.4,54.5,57.02},{54.4,53.16,51.8, 54.5},{53.16,52.1 ,50.34,51.8},{52.1,51.52,49.4 ,50.34},{51.52,49.36 ,47.62 ,49.4},{49.36,48.48,46.63,47.62},{48.48,48.65,45.51,46.63},{48.65,44.32,43.78,45.51},{44.32,39.58,40.3 ,43.78},{39.58,37.28,38.54,40.3 },{37.28,35.62,37.36,38.54},{35.62,34.22,36.1,37.36} ,{34.22,33.04,35.12,36.1},{33.04,31.42,33.56,35.12},{31.42,30.88,32.6,33.56},{30.88,30.08,29.78,32.6}, {30.08,28.56,27.4,29.78},{28.56,17.42,15.66,27.4},{17.42,10,10,15.66} };
...
GLfloat rectangle_blockX[NUM_BLOCK][4] = { { -4.38,-5.36,-5.36,-4.38 }, { -17.7,-18.1,-18.1,-17.7 },{-35.5,-36.7,-36.7,-35.5} };
GLfloat rectangle_blockY[NUM_BLOCK][4] = { { -1.66,-1.66,-0.53,-0.53 }, {-1.66,-1.66,-0.54,-0.54},{0.42,0.42,1.48,1.48} };
bool InFlag = false;
...
bool JudgeCollision(GLfloat x, GLfloat y)
{
	...
	{
		GLfloat p1_x = rectangle_blockX[i][0];
		GLfloat p1_y = rectangle_blockY[i][0];
		GLfloat p2_x = rectangle_blockX[i][1];
		GLfloat p2_y = rectangle_blockY[i][1];
		GLfloat p3_x = rectangle_blockX[i][2];
		GLfloat p3_y = rectangle_blockY[i][2];
		GLfloat p4_x = rectangle_blockX[i][3];
		GLfloat p4_y = rectangle_blockY[i][3];
...
void Ball::Roll()
{
	...
	if (!InFlag &&JudgeInside(center[0], center[1]) == false)
	{
		center[2] -= 1;
		if(deadTime == 1)
		std::cout << "game over!\n";
	...
	if (JudgeCollision(center[0], center[1]) == true)
	{
